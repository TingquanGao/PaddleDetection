...
from .read_order_transformer import ReadOrderTransformer
...
def _get_pred_class_order_and_mask(query_embed,
                             mask_feat,
                             dec_norm,
                             score_head,
                             order_head,
    ...
    out_query = dec_norm(query_embed)
    out_logits = score_head(out_query)
    valid_query = out_query[:, -num_queries:] if num_queries is not None else out_query
    order_logits = order_head(valid_query)
    mask_query_embed = mask_query_head(out_query)
    batch_size, mask_dim, _ = mask_query_embed.shape
    _, _, mask_h, mask_w = mask_feat.shape
    out_mask = paddle.bmm(
        mask_query_embed, mask_feat.flatten(2)).reshape(
        [batch_size, mask_dim, mask_h, mask_w])
    return out_logits, order_logits, out_mask


class PPDocLayoutV3TransformerDecoder(nn.Layer):
    def __init__(self,
                 hidden_dim,
                 decoder_layer,
                 num_layers,
                 ...
                 eval_idx=-1,
                 eval_topk=100):
        super(PPDocLayoutV3TransformerDecoder, self).__init__()
...
class PPDocLayoutV3(nn.Layer):
    ...
    def __init__(self,
                 ...
                 eval_idx=-1,
                 eps=1e-2):
        super(PPDocLayoutV3, self).__init__()
        ...
        decoder_layer = TransformerDecoderLayer(
            ...
            num_decoder_points)
        self.decoder = PPDocLayoutV3TransformerDecoder(hidden_dim, decoder_layer,
        ...
        # reading order prediction heads
        self.dec_order_head = nn.LayerList([nn.Linear(hidden_dim, hidden_dim) for _ in range(num_decoder_layers)])
        self.dec_global_pointer = GlobalPointerPD(hidden_size=hidden_dim, head_size=64)
    ...
    def _reset_parameters(self):
        ...
        # order head init
        for order_ in self.dec_order_head:
            linear_init_(order_)
            constant_(order_.bias, bias_cls)
    ...
    def forward(self, feats, pad_mask=None, gt_meta=None, is_teacher=False):
        ...
        return out_logits, out_bboxes, out_masks, out_order_logits, enc_out, init_out, dn_meta
